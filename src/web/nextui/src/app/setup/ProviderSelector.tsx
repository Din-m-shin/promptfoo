...
const defaultProviders: ProviderOptions[] = ([] as (ProviderOptions & { id: string })[])
  ...
  .concat(
    ['a770:Enter-the-model-name-and-baseURL-in-API-keys-settings'].map((id) => ({
      id,
      config: {
        temperature: 0.7,
        top_p: 0.1,
  ...
  .concat(
    ['a6000:Enter-the-model-name-and-baseURL-in-API-keys-settings'].map((id) => ({
      id,
      config: {
        temperature: 1,
        top_p: 0.1,
...
const ProviderSelector: React.FC<ProviderSelectorProps> = ({ providers, onChange }) => {
  ...
  const getProviderLabel = (provider: string | ProviderOptions): string => {
    ...
    if (provider.id && provider.id.startsWith('a6000:')) {
      if (
